"""End-to-end smoke run for Slice 2 (T2.1).

Wires StubCSI -> TinyCNN encoder -> SimCLR pre-training (no aug) ->
linear-probe eval. T2.1 only proves the pipeline runs; T2.3 will add the
static/dynamic decomposition, T2.5 the static-component perturbation
...
def main(
    ...
    print(f"[T2.1] encoder params: {count_parameters(encoder)}")
    ...
    losses = pretrain_simclr(
        ...
        lr=1e-3,
        temperature=0.5,
        augment_fn=None,  # T2.5 will plug in the static-perturbation aug
        device=device,
    )
    print(f"[T2.1] pre-train losses: {[round(loss, 4) for loss in losses]}")
    ...
    print(
        f"[T2.1] linear-probe accuracy on stub data: {acc:.3f} (chance ~ {chance:.3f})"
    )
    return acc
