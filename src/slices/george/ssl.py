"""SimCLR wrapper, projection head, and NT-Xent loss.

T1.1 only needs the pipeline to run end-to-end on stub data. The augmentation
function is left as a hook (`augment_fn`) so T1.3 (generic baseline) and T1.4
(Doppler-aware time warping) can plug in without touching this file.
"""
...
def make_views(
    ...
    `batch` is a CSI batch shaped (B, T, S, A). Each call to `augment_fn` should
    sample its own randomness; the two views may differ even when the same
    function is used for both. T1.1 passes `augment_fn=None` (identity views) —
    the contrastive signal still works because in-batch negatives push other
    samples apart.
    """
...
def pretrain_simclr(
    ...
    *,
    ...
    """Run SimCLR pre-training. Returns the list of per-epoch mean losses."""
    model.to(device)
    model.train()
