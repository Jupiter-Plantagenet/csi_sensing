...
T3.1 only needs the pipeline to run end-to-end on stub data. The augmentation
function is left as a hook (`augment_fn`) so T3.5 (phase-noise injection) and
the T3.6 generic baseline can plug in without touching this file.
"""
...
def make_views(
    ...
    function is used for both. T3.1 passes `augment_fn=None` (identity views) —
